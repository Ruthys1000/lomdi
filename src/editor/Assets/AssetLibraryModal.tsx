...
export function AssetLibraryModal({ open, onClose, onPick, kind }: AssetLibraryModalProps) {
  ...
  return (
    <dialog
      ...
      className="m-auto w-[min(52rem,calc(100vw-2rem))] rounded-2xl border border-sand-200 p-0 shadow-2xl backdrop:bg-sand-900/40"
    >
      <div className="flex items-center gap-3 border-b border-sand-100 px-5 py-4">
        ...
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full rounded-lg border border-sand-200 py-1.5 ps-9 pe-3 text-sm focus:border-clay-500 focus:ring-1 focus:ring-clay-500 focus:outline-none"
          />
        </div>
        ...
        <button
          ...
          className="inline-flex items-center gap-1.5 rounded-lg bg-clay-600 px-3 py-1.5 text-sm font-semibold text-white transition hover:bg-clay-700 disabled:opacity-50"
        >
      ...
      {errors.length > 0 && (
        <ul className="border-b border-ochre-100 bg-ochre-50 px-5 py-3 text-xs leading-relaxed text-ochre-800">
          {errors.map((error) => (
      ...
      <div
        className={cn(
          ...
          dragging && 'bg-clay-50/60 ring-2 ring-inset ring-clay-400',
        )}
      >
        {visible.length === 0 ? (
          <div className="flex h-56 flex-col items-center justify-center gap-2 rounded-xl border border-dashed border-sand-200 text-center">
            <ImageIcon className="size-8 text-sand-300" aria-hidden />
            <p className="text-sm font-semibold text-sand-500">
              {assets.length === 0 ? 'עדיין אין נכסים בפרויקט' : 'אין נכס שתואם את החיפוש'}
            </p>
            <p className="max-w-sm text-xs leading-relaxed text-sand-400">
              ...
              return (
                <li key={asset.id} className="group relative">
                  <button
                    ...
                    disabled={!onPick}
                    className={cn(
                      'block w-full overflow-hidden rounded-xl border border-sand-200 bg-white text-start transition',
                      onPick
                        ? 'hover:-translate-y-0.5 hover:border-clay-400 hover:shadow-md'
                        : 'cursor-default',
                    )}
                  >
                    <span className="flex h-28 items-center justify-center bg-sand-50">
                      {asset.kind === 'image' && url ? (
                        ...
                        <Film className="size-7 text-sand-300" aria-hidden />
                      )}
                    </span>

                    <span className="block px-2.5 py-2">
                      <span className="block truncate text-xs font-semibold text-sand-800">
                        {asset.originalName}
                      </span>
                      <span className="mt-0.5 flex items-center gap-1.5 text-[11px] text-sand-400">
                      ...
                      {unused && (
                        <span className="mt-1.5 inline-block rounded-md bg-sand-100 px-1.5 py-0.5 text-[10px] font-medium text-sand-500">
                          לא בשימוש
                        </span>
                      )}
                    </span>
                  ...
                  <button
                    ...
                    className="absolute end-1.5 top-1.5 rounded-lg bg-white/90 p-1.5 text-sand-400 opacity-0 shadow-sm transition group-hover:opacity-100 hover:text-plum-600 focus:opacity-100"
                  >
...
function DeleteAssetConfirm({
  ...
  return (
    <div
      ...
      className="border-t border-sand-100 bg-sand-50 px-5 py-4"
    >
      <p className="text-sm font-semibold text-sand-900">מחיקת "{asset.originalName}"?</p>
      ...
      <div className="mt-3 flex gap-2">
        <button
          type="button"
          onClick={onConfirm}
          className="rounded-lg bg-plum-600 px-3 py-1.5 text-xs font-semibold text-white hover:bg-plum-700"
        >
          מחיקה
        </button>
        <button
          type="button"
          onClick={onCancel}
          className="rounded-lg border border-sand-200 bg-white px-3 py-1.5 text-xs font-semibold text-sand-700 hover:bg-sand-50"
        >
          ביטול
        </button>
      </div>
