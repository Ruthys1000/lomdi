 ...
 * והלומדה מופיעה במגירת "הלומדות שלי" בראש הרשימה (saveNow מעדכן גם את
 * lastProjectId, שקובע את המיון שם).
 ...
 * מעולם אינה קיימת באחסון — רענון מחק אותה, והיא לא הופיעה במגירת "הלומדות
 * שלי". `openProjectFile` כבר עשה את זה; רק המסלול הזה פוספס.
 */
