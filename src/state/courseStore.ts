...
import type { BlockContent, BlockSettings, Chapter, Course, Theme } from '@/model/types';
 ...
 * מנגנון ה-undo (zundo) יתחבר לכאן בשלב 4.
 */
...
interface CourseState {
  ...
  updateChapter: (chapterId: string, patch: Partial<Omit<Chapter, 'id' | 'blocks'>>) => void;
  removeChapter: (chapterId: string) => void;
  duplicateChapter: (chapterId: string) => void;
  moveChapter: (from: number, to: number) => void;

  addBlock: (chapterId: string, type: string, atIndex?: number) => string | undefined;
  ...
  moveBlockToChapter: (blockId: string, targetChapterId: string, targetIndex: number) => void;
...
export const useCourseStore = create<CourseState>((set, get) => {
  ...
  return {
    ...
    loadCourse: (course) => set({ course }),
    closeCourse: () => set({ course: null }),

    updateCourse: (patch) => mutate((course) => ops.updateCourse(course, patch)),
    ...
    removeChapter: (chapterId) => mutate((course) => ops.removeChapter(course, chapterId)),

    duplicateChapter: (chapterId) =>
      mutate((course) => ops.duplicateChapterById(course, chapterId)),

    moveChapter: (from, to) => mutate((course) => ops.moveChapter(course, from, to)),
    ...
    removeBlock: (blockId) => mutate((course) => ops.removeBlock(course, blockId)),
    ...
    moveBlockWithinChapter: (chapterId, from, to) =>
      mutate((course) => ops.moveBlockWithinChapter(course, chapterId, from, to)),

    moveBlockToChapter: (blockId, targetChapterId, targetIndex) =>
      mutate((course) => ops.moveBlockToChapter(course, blockId, targetChapterId, targetIndex)),

    nudgeBlock: (blockId, direction) => mutate((course) => ops.nudgeBlock(course, blockId, direction)),
  };
});
